...
class logstic_regression_model(nn.Module):
    ...
    def forward(self, score):
        logit = self.w * score + self.b

        logit_probability = 1/(1+torch.exp(-logit)) 
        ne_probability = 1-logit_probability
        probability = torch.cat((ne_probability, logit_probability),dim=0)
...
def train_model(score, non_genetic_data, label, num_factors, model_save, loss_save, PRS_method, disease, alpha=1,beta=1,delta=1,batch_size=100,n_epoch=1000,n_cols_factor=None):    
    ...
    #for epoch in range(2): # for debug
        ...
        for i in batches:
            ...
            prob_re_results, logit_re_result = genetic(batch_PRS)
            prob_re_results = prob_re_results.T
            prob_linear_results, logit_linear_results = non_genetic(batch_data)
 
            ## loss & backward 
            logistic_re_loss = F.cross_entropy(prob_re_results, batch_y)
            log_linear_loss = F.cross_entropy(prob_linear_results, batch_y)
            kl_loss = util.kl_loss(prob_linear_results, prob_re_results)  
...
def validation_loss(logstic_model, loglinear_model, score ,non_genetic_data, label, alpha, beta, batch_size):
    ...
    val_prob_re_results, _ = logstic_model(score['eval'])
    val_prob_re_results = val_prob_re_results.T
    val_prob_linear_results, _ = loglinear_model(non_genetic_data['eval'])

    # validation loss  
    val_loss_non_genetic = F.cross_entropy(val_prob_linear_results, label['eval'])
    val_loss_genetic = F.cross_entropy(val_prob_re_results, label['eval'])
    val_loss_kl = util.kl_loss(val_prob_linear_results, val_prob_re_results)  
...
def validate_best_alpha_beta(model_save, PRS_method, disease, score, non_genetic_data, label, alpha, beta,  num_factors, n_cols_factor):
    ...
    for i in range(len(alpha)):
        ...
        for j in range(len(beta)):
            ...
            prob_re_results, logit_re_result = val_genetic(score['eval'])
            prob_re_results, logit_re_result = prob_re_results.T, logit_re_result.T
        
            prob_linear_results, logit_linear_results = val_non_genetic(non_genetic_data['eval'])

            # y_pred_score = logit_re_result[:,0] + logit_linear_results[:,1]
            y_pred_score = prob_re_results[:,1] + prob_linear_results[:,1]
            
            #y_pred_class  =  y_pred_class.detach().numpy()
            y_pred_score  =  y_pred_score.detach().numpy()
            y = label['eval'].detach().numpy() 
              
            #AUROC = roc_auc_score(y, y_pred_class) 
            AUROC = roc_auc_score(y, y_pred_score)
            print(f'alpha:\t {alpha[i]}; beta:\t {beta[j]}; AUROC:\t {AUROC}')       
...
def test_model(model_save, PRS_method, disease, score, non_genetic_data, label, alpha, beta,  num_factors, n_cols_factor):    
    ...
    prob_re_results, logit_re_result = test_genetic(score['eval'])
    prob_re_results, logit_re_result = prob_re_results.T, logit_re_result.T

    prob_linear_results, logit_linear_results = test_non_genetic(non_genetic_data['eval'])
...
def loding_data(PRS_score_path, PRS_Method, Disease, non_genetic_data_path, label_path, med_path):
    score_dict = {}
    non_genetic_data_dict = {}
    label_dict = {}
    valt = {'train':'train','eval':'eval'}
    for _set in ['train','eval']:
        # disease_train/eval_prs.txt
        score = pd.read_table(os.path.join(PRS_score_path, f'{_set}_prs.txt'), sep=' ')['prs'].to_numpy()
        score_dict[_set] =  Variable(torch.FloatTensor(score))
        
        # disease non-genetic data
        data = pd.read_table(os.path.join(non_genetic_data_path, f'processed_{_set}_data.txt'))
        data['Age at attend assessment center'] /= 10 # used age group
        data['Age at attend assessment center'] = data['Age at attend assessment center'].astype('int')
        
        # disease med data
        if med_path is not None:
            med_data = np.load(os.path.join(med_path, f'{_set}_med_output.npy'))
            med_colname = ['med:'+str(i) for i in range(med_data.shape[1])]
            med_data = pd.DataFrame(med_data, columns=med_colname)

            data = pd.concat([data, med_data], axis=1)

        #non_genetic_data_dict[_set] = Variable(torch.FloatTensor(data))
        non_genetic_data_dict[_set] = data
      
        y = np.genfromtxt(os.path.join(label_path, f'{_set}_label.txt'))      
...
def get_data_info_for_model_construction(non_genetic_data):    
    ...
    print(f'n_cols_factor:\t{n_cols_factor}')           
...
if __name__ == '__main__':
    ...
    ### user input params
    Disease = 'CAD'
    PRS_score_path = f'./PRS/LDPred2/{Disease}'
    non_genetic_data_path = f'./non_genetic_data/{Disease}/model_needed_data'
    label_path = f'./non_genetic_data/{Disease}/model_needed_data'
    med_path = f'./Med-Transformer/extracted_info/{Disease}'
    category_list =['lifestyle','physical_measures']#'physical_measures' 'lifestyle'
    ...
    ### data loading
    score, non_genetic_data, label = loding_data(PRS_score_path, 
                                                 PRS_Method, 
                                                 Disease, 
                                                 non_genetic_data_path, 
                                                 label_path,
                                                 med_path) 
    n_factors, n_cols_factor= get_data_info_for_model_construction(non_genetic_data)
    ...
    for i in range(len(alpha_grid)):
        for j in range(len(beta_grid)):
            ...
            if os.path.exists(os.path.join(model_dir, 'model_'+ PRS_Method + '_' + Disease + '_' + str(alpha_grid[i])+'_'+str(beta_grid[j]) +'.pth')):
